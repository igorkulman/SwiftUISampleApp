...
//  Core
//
//  Created by Igor Kulman on 19.11.2024.
//
...
public struct DependencyValues {
    ...
    public static subscript<K>(
        key: K.Type
    ) -> K.Value where K: DependencyKey {
        get { key.currentValue }
        set { key.currentValue = newValue }
    }

    public static subscript<T>(
        _ keyPath: WritableKeyPath<DependencyValues, T>
    ) -> T {
...
public extension View {
    func dependency<Value>(
        ...
        DependencyValues[keyPath] = value
        return self
    }
}
